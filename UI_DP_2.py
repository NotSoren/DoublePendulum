...
def vel(x1,y1,x2,y2,t):
    return(math.sqrt(float(x1-x2)**2 + float(y1-y2)**2)/t)
...
def main():
    ...
    pygame.init()
    #pygame.font.init()
    #myfont = pygame.font.SysFont('Mono', 12)
    ...
    while running:
        ...
        if place%12==0:curr_speed = vel(x_2,y_2,x_p,y_p,h)
        if curr_speed != speed: print(curr_speed); speed = curr_speed
        x_p = x_2
        y_p = y_2
